...
function calculateDaysToBirhtday(personToCalculate) {
  ...
  let nextBirthday = (0, _dateFns.setYear)(birthday, today.getFullYear()); // if the date is already behind us, we add + 1 to the year
...
},{"date-fns":"node_modules/date-fns/esm/index.js"}],"people.json":[function(require,module,exports) {
module.exports = [{
...
async function fetchPerson() {
  ...
  function filterByName(peopleToFilter) {
    if (_elements.filterSearchInput.value !== "") {
      return peopleToFilter.filter(person => {
        const fullNameLowercase = person.firstName.toLowerCase() + ' ' + person.lastName.toLowerCase();
        return fullNameLowercase.includes(_elements.filterSearchInput.value.toLowerCase());
      });
    }
  ...
  function filterBothNameAndMonth() {
    ...
    _elements.list.innerHTML = filteredPeople;
  ...
  async function destroyPopup(popup) {
    await setTimeOut(100); // remove it from the DOM
  ...
  const handleClick = e => {
    ...
    if (deleteBtn) {
      ...
      `);
        document.body.appendChild(div); //put a very small titmeout before we add the open class
  ...
  _elements.addBtn.addEventListener('click', addPerson);
  ...
  _elements.filterSearchInput.addEventListener('keyup', () => filterBothNameAndMonth());

  _elements.filterMonthInput.addEventListener('change', () => filterBothNameAndMonth());
...
;
fetchPerson();
},{"./elements.js":"elements.js","./displayList":"displayList.js","./people.json":"people.json"}],"../../AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
